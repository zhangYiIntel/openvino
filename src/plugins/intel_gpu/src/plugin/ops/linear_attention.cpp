...
using LinearAttention = ov::intel_gpu::op::LinearAttention;
}  // namespace internal_gpu
}  // namespace op
}  // namespace ov
...
static void CreateLinearAttentionOp(ProgramBuilder& p, const std::shared_ptr<ov::intel_gpu::op::LinearAttention>& op) {
    validate_inputs_count(op, {6});
...
REGISTER_FACTORY_IMPL(internal_gpu, LinearAttention);
