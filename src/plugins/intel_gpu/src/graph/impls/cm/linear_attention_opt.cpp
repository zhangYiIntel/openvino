...
class LinearAttentionOptImpl : public PrimitiveImplCM {
public:
    DECLARE_OBJECT_TYPE_SERIALIZATION(ov::intel_gpu::cm::LinearAttentionOptImpl)

    Stage::Ptr linear_attn = make_stage<LinearAttentionGenerator>();

    LinearAttentionOptImpl() : PrimitiveImplOCL(LinearAttentionOptImplementationManager::get_type_info_static()) {
    }
...
std::unique_ptr<primitive_impl> LinearAttentionOptImplementationManager::create_impl(const program_node& node, const RuntimeParams& params) const {
...
BIND_BINARY_BUFFER_WITH_TYPE(cldnn::linear_attention)
BIND_BINARY_BUFFER_WITH_TYPE(ov::intel_gpu::cm::LinearAttentionOptImpl)
