...
layout paged_gated_delta_net_inst::calc_output_layout(const paged_gated_delta_net_node& node,
                                                       const kernel_impl_params& impl_param) {
    return calc_output_layouts<ov::PartialShape>(node, impl_param)[0];
}

template <typename ShapeType>
std::vector<layout> paged_gated_delta_net_inst::calc_output_layouts(const paged_gated_delta_net_node& node,
                                                                     const kernel_impl_params& impl_param) {
    ...
    if (out_ps.rank().is_static() && q_ps.rank().is_static() && out_ps.rank().get_length() == 3 && q_ps.rank().get_length() == 3) {
        out_ps[0] = q_ps[0];
    }

    return {layout(out_ps, value_layout.data_type, value_layout.format)};
}

template std::vector<layout>
paged_gated_delta_net_inst::calc_output_layouts<ov::PartialShape>(const paged_gated_delta_net_node& node,
                                                                   const kernel_impl_params& impl_param);
...
paged_gated_delta_net_inst::typed_primitive_inst(network& network, const paged_gated_delta_net_node& node)
    : parent(network, node) {}
