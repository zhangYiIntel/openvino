...
using RecurrentLinearAttnPagedParams = std::tuple<int32_t, int32_t, int32_t>;  // head_size, seq_length, num_sequences
...
TEST_P(PagedGatedDeltaNetKernelTest, MatchesReferenceAndUpdatesState) {
    ...
    PlainTensor query_t;
    query_t.resize({static_cast<size_t>(tokens), static_cast<size_t>(qk_heads), static_cast<size_t>(head_size)}, query.data());
    PlainTensor key_t;
    key_t.resize({static_cast<size_t>(tokens), static_cast<size_t>(qk_heads), static_cast<size_t>(head_size)}, key.data());
    PlainTensor value_t;
    value_t.resize({static_cast<size_t>(tokens), static_cast<size_t>(v_heads), static_cast<size_t>(head_size)}, value.data());
    PlainTensor state_t;
    state_t.resize({block_indices.size(), static_cast<size_t>(v_heads), static_cast<size_t>(head_size), static_cast<size_t>(head_size)},
    ...
    output_t.resize({static_cast<size_t>(tokens), static_cast<size_t>(v_heads), static_cast<size_t>(head_size)}, output.data());
