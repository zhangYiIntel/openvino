...
struct MHAHelper {
    ...
    ov::Extensions::Cpu::PagedAttnExecutorParams _params;
...
#    endif
    ...
    explicit MHAHelper(ov::Extensions::Cpu::PagedAttnExecutorParams params)
        : _key_group_size(params.key_group_size),
          _value_group_size(params.value_group_size),
          _quant_key_bychannel(params.quant_key_bychannel),
          _quant_value_bychannel(params.quant_value_bychannel),
          _params(params) {
        _weight.resize<float>({size_t{1}, size_t{1}, size_t{1}, size_t{1}});
    }
        ...
        for (size_t h = hq_beg; h < hq_end; h++) {
            ...
            for (size_t m = q_start; m < q_end; m++) {
                ...
                if (_sliding_window) {
                    ...
                    std::cout <<  "exec_kernel_multiple|before_softmax|ncausal|" << ncausal << "|cur_kv_len|" << cur_kv_len << "|"  << 
                        _weight.slice(0, 0, 1) << "|alibi_slope|" << alibi_slope << "|alibi_lookup|" << alibi_lookup << std::endl;
                    attn_softmax_kernel<float>(score,
                                               ...
                                               precision_of<DATA_TYPE>::value,
                                               alibi_slope);
                    std::cout <<  "exec_kernel_multiple|after_softmax|"  << _weight.slice(0, 0, 1) << std::endl;
                }
                if (score_output) {
            ...
            std::cout <<  "exec_kernel_multiple|weight|"  << _weight.slice(0, 0, 1) << std::endl;
            ...
            if (q_is_xf16) {
                std::cout << "Multiple|_output|" << _output << std::endl;
                attn_memcpy2d_kernel(_output.ptr<float>(ithr, 0, h, 0),
                                     ...
                                     SV,
                                     q_cnt);
                std::cout << "Multiple|output_emb|" << output_emb << std::endl;
            }
        }
    }
...
#    endif
        ...
        std::cout << "ExecuteBHL|" << output_emb << std::endl;
    }
};
        ...
        parallel_for2d_dynamic(attn_work_count, loop_hk ? Hk : _helper.H, [&](size_t w, size_t hx) {
            ...
            if (q_len == 1) {
                ...
                _helper.exec_kernel_one_bh(
                    ...
                    alibi_slopes,
                    score_output);
                std::cout << "ExecuteBH|" << "cur_kv_len" << cur_kv_len << "|" << output_emb << std::endl;
            } else {
                const auto batch_in_reorder = item.batch_in_reorder;
...
struct AttentionExecutor : public PagedAttentionExecutor {
    ...
    void init(const std::vector<MemoryPtr>& inputs,
        ...
        if (inputs_size > ID_ROTATED_BLOCK_INDICES && !_helper._params.fuse_rope) {
    ...
    void execute_rope(std::vector<PlainTensor>& inputs, std::vector<PlainTensor>& outputs) {
        _rope_executor_ptr->execute(inputs, outputs);
    }
