...
std::shared_ptr<ov::intel_cpu::paged_attn::RopeExecutor> ov::intel_cpu::paged_attn::make_rope_executor(
    ...
    //         ptr = std::make_shared<RoPEExecutorQwen<ov::float16>>(config);
    //     } else if (rtPrecision == ov::element::bf16) {
    //         ptr = std::make_shared<RoPEExecutorQwen<ov::bfloat16>>(config);
    //     } else {
    //         ptr = std::make_shared<RoPEExecutorQwen<float>>(config);
