...
DumpModel::DumpModel(std::string _file_name) {
    ...
    static const char* dump_enabled = std::getenv("OV_CPU_DUMP_MODELS");
    if (!dump_enabled || dump_enabled[0] != '1')
        return;
...
bool DumpModel::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    std::cout << "[OV_CPU_DUMP_MODELS] Model is dumpped into " << file_name << " and " << file_name << ".xml" << std::endl;
    return false;
}
