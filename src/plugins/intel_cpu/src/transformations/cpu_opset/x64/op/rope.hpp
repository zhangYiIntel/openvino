...
public:
    ...
    struct Config {
        size_t reshape_H = 0;           // split last dimension of 3D input into [H,?]
        size_t slice_start = 0;         // slice inner-most dimensions of input
        size_t slice_stop = 0;
        bool input_trans0213 = false;   // transpose input before *cos/*sin
        bool is_cos_sin_combined = false; // is_cos_sin_combined
        bool is_interleaved = false;    // use orginal formula in interleaved form
