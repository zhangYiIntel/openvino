...
ov::pass::RoPEFusionChatGLM4::RoPEFusionChatGLM4(int split_output_id) {
    ...
    std::cout << "Add ChatGLM4|" << split_output_id << std::endl;
    auto qkv_linear = makePattern("[?,?,?]");  //  [seq_length, batch_size, 4608]
    auto seq_length = makePattern("i32[1]");
    auto cos_sin_cache = makePattern("[?,?,?,?]");  // [max_pos_embeddings, batch_size, 32, 2]
    ...
    auto var_split_1 = makePattern<opset1::VariadicSplit>({cur_key_transposed, 3, {ndims, ov::gen_pattern::Symbol("end")}}, {}, nullptr, "var_split_1");
    var_split_1->set_output_size(2);
    ...
    auto const_x_shape = makeConst({0, head_cnt, 0, ndims / 2, 2});
    auto reshape_x_1 = makePattern<opset1::Reshape>(
        {slice_Slice_437 | var_split_1->output(0), ListConstruct_452_Concat | const_target_shape_1});
    auto reshape_x_2 = makePattern<opset1::Reshape>(
        {slice_Slice_437 | var_split_1->output(0), const_x_shape}, {{"special_zero", true}});
    auto reshape_Reshape_453 = reshape_x_1 | reshape_x_2;
    auto x_even = makePattern<opset8::Gather>({reshape_Reshape_453, 0, -1}, {{"batch_dims", 0}}, nullptr, "x_even");
    auto x_odd = makePattern<opset8::Gather>({reshape_Reshape_453, 1, -1}, {{"batch_dims", 0}}, nullptr, "x_odd");

    auto cos_sin_shape = makeOP<opset1::Concat>({{-1}, {1}, seq_len, {ndims / 2}, {2}}, {{"axis", 0}});

    auto ScatterUpdate = makePattern<opset3::ScatterUpdate>({{0, 0}, {1}, seq_len, {0}}, {}, nullptr, "ScatterUpdate");
    auto cos_sin_slice = makePattern<ov::opset8::Slice>({cos_sin_cache, {0}, seq_length, {1}, {1}}, {}, nullptr, "cos_sin_slice");
    auto slice_Slice_449 = makePattern<ov::opset8::Slice>({cos_sin_cache, {0,0}, ScatterUpdate, {1,1}, {0}}, {}, nullptr, "slice_Slice_449");
    auto slice_StridedSlice_449 = GenStridedSlice(cos_sin_cache, {0,0}, ScatterUpdate, {1,1}, 1);
    auto var_split_2 = makePattern<opset1::VariadicSplit>({cos_sin_cache, 0, {0, ov::gen_pattern::Symbol("end")}}, {}, nullptr, "var_split_2");
    var_split_2->set_output_size(2);

    auto view_Reshape_460 =
        makePattern<opset1::Reshape>({slice_StridedSlice_449 | slice_Slice_449 | var_split_2->output(0) | cos_sin_slice,
                                      ListConstruct_379_Concat | cos_sin_shape | const_target_shape_2},
                                     {{"special_zero", false}}, nullptr, "view_Reshape_460");

    auto cos_tab = makePattern<opset8::Gather>({view_Reshape_460, 0, -1}, {{"batch_dims", 0}}, nullptr, "cos_tab");
    auto x_even_cos = makePattern<opset1::Multiply>({x_even, cos_tab}, {{"auto_broadcast", "numpy"}}, nullptr, "x_even_cos");

    auto sin_tab = makePattern<opset8::Gather>({view_Reshape_460, 1, -1}, {{"batch_dims", 0}}, nullptr, "sin_tab");
    auto x_odd_sin = makePattern<opset1::Multiply>({x_odd, sin_tab}, {{"auto_broadcast", "numpy"}}, nullptr, "x_odd_sin");
    auto neg_x_odd_sin = makePattern<opset1::Multiply>({x_odd_sin, -1.000000f}, {{"auto_broadcast", "numpy"}}, nullptr, "neg_x_odd_sin");
    auto sub_Subtract_469 = makePattern<opset1::Add>({x_even_cos, neg_x_odd_sin}, {{"auto_broadcast", "numpy"}}, nullptr, "sub_Subtract_469");

    auto y_even = makePattern<opset1::Unsqueeze>({sub_Subtract_469, -1}, {}, nullptr, "y_even");
    auto x_odd_cos = makePattern<opset1::Multiply>({x_odd, cos_tab}, {{"auto_broadcast", "numpy"}}, nullptr, "x_odd_cos");
    auto x_even_sin = makePattern<opset1::Multiply>({x_even, sin_tab}, {{"auto_broadcast", "numpy"}}, nullptr, "x_even_sin");
    auto add_Add_476 = makePattern<opset1::Add>({x_odd_cos, x_even_sin}, {{"auto_broadcast", "numpy"}}, nullptr, "add_Add_476");
    auto y_odd = makePattern<opset1::Unsqueeze>({add_Add_476, -1}, {}, nullptr, "y_odd");

    auto stack_481 = makePattern<opset1::Concat>({y_even, y_odd}, {{"axis", -1}}, nullptr, "stack_481");
    ...
    auto m = std::make_shared<ov::pass::pattern::Matcher>(cat_Concat_505, matcher_name);
    this->register_matcher(m, callback);
}
